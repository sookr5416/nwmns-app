...
import CustomPopup, { PopupState } from '../../components/CustomPopup'; // 🌟 공통 팝업 임포트
...
export default function AdminCourtsPage() {
  ...
  const handleRegister = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (isDuplicate) return showPopup('alert', '중복 등록', '이미 등록된 선수가 있습니다.');
          ...
          {isEditingInfo ? (
            <div className="flex items-center gap-2 flex-wrap">
              <input 
                ...
                placeholder="장소 입력"
                className="text-xs font-bold text-slate-800 border border-indigo-500 rounded px-2 py-1 outline-none w-44"
              />
              <input 
                type="text" 
                value={timeTitle}
                ...
                placeholder="시간 입력"
                className="text-xs font-bold text-slate-800 border border-indigo-500 rounded px-2 py-1 outline-none w-28"
              />
              <input 
                type="text" 
                value={memoTitle}
              ...
              <button 
                ...
                className="bg-indigo-600 text-white text-xs font-bold px-3 py-1 rounded hover:bg-indigo-700"
              >
                저장
              </button>
            </div>
            ...
            <h1 
              ...
              className="text-sm font-extrabold text-slate-800 cursor-pointer hover:text-indigo-600 transition-colors flex items-center gap-2"
              title="클릭하여 장소, 시간, 비고 수정"
            >
