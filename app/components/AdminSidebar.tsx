...
import { Player } from '@/types'; // 공용 타입 불러오기

/* ==================================
     Props 인터페이스 정의
     부모 컴포넌트(page.tsx)로부터 전달받는 상태와 함수들의 타입을 정의
     ================================== */
interface AdminSidebarProps {
  ...
  // DOM 제어용 Ref (등록 후 이름 칸으로 자동 포커스)
  nameInputRef: RefObject<HTMLInputElement | null>;

  // 주요 비즈니스 로직 함수들
  handleRegister: (e: FormEvent<HTMLFormElement>) => void;  // 등록 버튼 클릭
  players: Player[];                                        // 전체 선수 목록 데이터
  selectedPlayerId: string | null;                          // 현재 클릭된 선수 ID

  // 상호작용 (클릭 및 드래그 앤 드롭) 이벤트 핸들러
...
export default function AdminSidebar({
  ...
  return (
    ...
    // 사이드바 전체 컨테이너
    // order-2 md:order-1: 모바일에서는 아래로, PC에서는 왼쪽으로 강제 고정
    <div className="order-2 md:order-1 w-full md:w-83 h-[45vh] md:h-full flex-shrink-0 bg-white border-t md:border-t-0 md:border-r border-slate-200 flex flex-col shadow-[0_-5px_15px_rgba(0,0,0,0.05)] md:shadow-xl z-20">
      ...
      <div className="p-4 md:p-6 border-b border-slate-100">
        ...
        {isRegOpen && (
          <form onSubmit={handleRegister} className="space-y-4 mt-5">
            ...
            { /* 성별 선택 버튼 (클릭 시 UI 즉각 반영) */ }
            <div className="flex gap-2">
              {['남', '여'].map((g) => (
                <button key={g} type="button" onClick={() => setGender(g)} 
                className={`flex-1 py-2 rounded-lg font-medium transition-colors ${gender === g ? 'bg-indigo-100 text-indigo-700 border-2 border-indigo-500' : 'bg-slate-50 text-slate-500 border border-slate-200 hover:bg-slate-100'}`}>{g}</button>
              ))}
            </div>
      ...
      {/* ==================================
        하단 영역: 전체 대기 선수 (로비) 목록
        ================================== */}

        { /* 로비 빈 공간을 누르면 선택된 선수가 로비로 */ }
      <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50" onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, 'lobby')} onClick={() => handleSlotClick('lobby')}>
        ...
        <ul className="space-y-2 min-h-[200px]">
          {players
            ...
            .sort((a, b) => a.count - b.count)  // 게임 수가 적은 사람을 먼저 조
            .map((player) => (                  // 개별 선수 카드 (드래그 앤 드롭 및 클릭 이벶벤트 설정)
