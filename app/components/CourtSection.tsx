      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-4">
        ...
        <div className="flex items-center gap-2 md:gap-4 self-end md:self-auto">
          {viewMode === 'admin' && handleDayClose && (
            <button onClick={handleDayClose} className="px-4 py-2 bg-red-500 hover:bg-red-600 text-white text-sm font-bold rounded-lg shadow-md shadow-red-200 transition-colors flex items-center gap-2 active:scale-95">
            ...
            return (
              <div key={slot.id} className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden transition-all hover:shadow-md">
                  ...
                  <div className="flex items-center gap-1.5 min-w-0 flex-1">
                    {viewMode === 'admin' && isGameCourt && handleCourtRenameChange && handleCourtRenameSave ? (
                      <input
                        ...
                        className="bg-transparent text-white font-bold text-base focus:outline-none border-b border-dashed border-white/50 w-24 px-1 placeholder-white/50 truncate"
                      />
                    ) : (
                      <h3 className="text-white font-bold text-base truncate">{slot.title}</h3>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {viewMode === 'admin' && isGameCourt && onDeleteCourt && (
                      <button
                        ...
                        className="text-[11px] font-bold text-red-300 hover:text-white bg-red-900/40 hover:bg-red-600 px-1.5 py-0.5 rounded transition-colors"
                        title="코트 삭제"
                      >
                        삭제
                      </button>
                ...
                {viewMode === 'admin' && resetSlot && finishGame && startGame && (
                  <div className="p-4 border-t border-slate-100 flex gap-2 bg-slate-50 mt-auto" onClick={(e) => e.stopPropagation()}>
                    <button onClick={() => resetSlot(slot.id)} disabled={!!slot.start_time} className={`flex-1 py-2 bg-white border rounded-lg font-medium transition-colors text-sm ${slot.start_time ? 'border-slate-200 text-slate-300 cursor-not-allowed bg-slate-50' : 'border-slate-300 text-slate-600 hover:bg-slate-100'}`}>
                      초기화
                    </button>
                    {isGameCourt && (
                      <button 
                      ...
                      >
                        {processingCourtId === slot.id ? '처리 중...' : slot.start_time ? (
                          <><span className="w-2 h-2 rounded-full bg-red-200 animate-pulse"></span>종료 ({formatTime(slot.start_time)})</>
                        ) : '경기 시작'}
                      </button>
                    )}
                  </div>
